...
transformed parameters {
  real<lower=0> p[N-lag];
  real<lower=0> pZ[N-lag];
  real<lower=0> ppZ[N-lag];
  real<lower=0> Z[N-lag];
  real logit_p[N-lag];  // reporting probability
  real<lower=0> sd_p;
  real<lower=0> Z_cumsum;
