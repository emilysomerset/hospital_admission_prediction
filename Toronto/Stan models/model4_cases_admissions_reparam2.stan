...
parameters {
  ...
  // Admissions
  vector[I] tau_logit_init;
  matrix[I, J-1] z_tau_logit;            // admission age-time probabilities. 
...
transformed parameters {
  ...
  tau_logit[,1] = tau_logit_init;
  for (j in 2:J)
    tau_logit[,j] = rho*tau_logit[,j - 1] + sd_tau * z_tau_logit[,j - 1];
...
model {
  ...
  for (j in 2:J) {
   ...
   // Prior on logit(tau)
  tau_logit_init ~ normal(-2,1);
  to_vector(z_tau_logit) ~ normal(0, 1);
