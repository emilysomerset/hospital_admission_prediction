...
Type objective_function<Type>::operator() ()
{
  ...
  nll -= dnorm(logit_pi(0), Type(0.0), Type(1.0), true);
  for (int j = 1; j < J; ++j) {
  nll -= dnorm(logit_pi(j), logit_pi(j - 1),sd_p, true);
  }
